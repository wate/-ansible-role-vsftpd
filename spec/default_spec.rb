...
describe file('/etc/vsftpd/vsftpd.conf') do
  ...
  if vsftpd_cfg.key?('anonymous_enable')
    anonymous_enable = vsftpd_cfg['anonymous_enable'] ? 'YES' : 'NO'
...
ftp_control_port = property['vsftpd_cfg']['listen_port'] || 21
describe port(ftp_control_port) do
  it { should be_listening }
end

is_pasv = property['vsftpd_cfg']['pasv_enable'] || true
unless is_pasv
  ftp_data_port = property['vsftpd_cfg'].key?('ftp_data_port') || 20
  describe port(ftp_data_port) do
    it { should be_listening }
  end
end
